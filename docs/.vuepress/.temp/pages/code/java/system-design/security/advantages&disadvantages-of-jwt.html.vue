<template><div><p>在 <a href="https://javaguide.cn/system-design/security/jwt-intro.html" target="_blank" rel="noopener noreferrer">JWT 基本概念详解<ExternalLinkIcon/></a>这篇文章中，我介绍了：</p>
<ul>
<li>什么是 JWT?</li>
<li>JWT 由哪些部分组成？</li>
...
<h2 id="jwt-的优势" tabindex="-1"><a class="header-anchor" href="#jwt-的优势" aria-hidden="true">#</a> JWT 的优势</h2>
<p>相比于 Session 认证的方式来说，使用 JWT 进行身份认证主要有下面 4 个优势。</p>
<h3 id="无状态" tabindex="-1"><a class="header-anchor" href="#无状态" aria-hidden="true">#</a> 无状态</h3>
...
<h3 id="有效避免了-csrf-攻击" tabindex="-1"><a class="header-anchor" href="#有效避免了-csrf-攻击" aria-hidden="true">#</a> 有效避免了 CSRF 攻击</h3>
...
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>a</span> <span class="token attr-name">src</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>http://www.mybank.com/Transfer?bankId=11&amp;money=10000<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>科学理财，年盈利率过万<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>a</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>CSRF 攻击需要依赖 Cookie ，Session 认证中 Cookie 中的 <code v-pre>SessionID</code> 是由浏览器发送到服务端的，只要发出请求，Cookie 就会被携带。借助这个特性，即使黑客无法获取你的 <code v-pre>SessionID</code>，只要让你误点攻击链接，就可以达到攻击效果。</p>
<p>另外，并不是必须点击链接才可以达到攻击效果，很多时候，只要你打开了某个页面，CSRF 攻击就会发生。</p>
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>img</span> <span class="token attr-name">src</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>http://www.mybank.com/Transfer?bankId=11&amp;money=10000<span class="token punctuation">"</span></span> <span class="token punctuation">/></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><strong>那为什么 JWT 不会存在这种问题呢？</strong></p>
...
<p>不过，这样也会存在 XSS 攻击的风险。为了避免 XSS 攻击，你可以选择将 JWT 存储在标记为<code v-pre>httpOnly</code> 的 Cookie 中。但是，这样又导致了你必须自己提供 CSRF 保护，因此，实际项目中我们通常也不会这么做。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="适合移动端应用" tabindex="-1"><a class="header-anchor" href="#适合移动端应用" aria-hidden="true">#</a> 适合移动端应用</h3>
<p>使用 Session 进行身份认证的话，需要保存一份信息在服务器端，而且这种方式会依赖到 Cookie（需要 Cookie 保存 <code v-pre>SessionId</code>），所以不适合移动端。</p>
<p>但是，使用 JWT 进行身份认证就不会存在这种问题，因为只要 JWT 可以被客户端存储就能够使用，而且 JWT 还可以跨语言使用。</p>
<h3 id="单点登录友好" tabindex="-1"><a class="header-anchor" href="#单点登录友好" aria-hidden="true">#</a> 单点登录友好</h3>
<p>使用 Session 进行身份认证的话，实现单点登录，需要我们把用户的 Session 信息保存在一台电脑上，并且还会遇到常见的 Cookie 跨域的问题。但是，使用 JWT 进行认证的话， JWT 被保存在客户端，不会存在这些问题。</p>
<h2 id="jwt-身份认证常见问题及解决办法" tabindex="-1"><a class="header-anchor" href="#jwt-身份认证常见问题及解决办法" aria-hidden="true">#</a> JWT 身份认证常见问题及解决办法</h2>
<h3 id="注销登录等场景下-jwt-还有效" tabindex="-1"><a class="header-anchor" href="#注销登录等场景下-jwt-还有效" aria-hidden="true">#</a> 注销登录等场景下 JWT 还有效</h3>
<p>与之类似的具体相关场景有：</p>
<ul>
<li>退出登录;</li>
<li>修改密码;</li>
...
<h3 id="jwt-的续签问题" tabindex="-1"><a class="header-anchor" href="#jwt-的续签问题" aria-hidden="true">#</a> JWT 的续签问题</h3>
...
<p>另外，不用 JWT 直接使用普通的 Token(随机生成，不包含具体的信息) 结合 Redis 来做身份认证也是可以的。我在 <a href="https://javaguide.cn/open-source-project/" target="_blank" rel="noopener noreferrer">「优质开源项目推荐」<ExternalLinkIcon/></a>第 8 期推荐过的 <a href="https://github.com/dromara/sa-JWT" target="_blank" rel="noopener noreferrer">Sa-Token<ExternalLinkIcon/></a> 这个项目是一个比较完善的 基于 JWT 的身份认证解决方案，支持自动续签、踢人下线、账号封禁、同端互斥登录等功能，感兴趣的朋友可以看看。</p>
<p><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/javaguide/system-design/jwt/image-20220609170714725.png" alt="" loading="lazy"></p>
<h2 id="参考" tabindex="-1"><a class="header-anchor" href="#参考" aria-hidden="true">#</a> 参考</h2>
<ul>
<li>JWT 超详细分析：<a href="https://learnku.com/articles/17883" target="_blank" rel="noopener noreferrer">https://learnku.com/articles/17883<ExternalLinkIcon/></a></li>
<li>How to log out when using JWT：<a href="https://medium.com/devgorilla/how-to-log-out-when-using-jwt-a8c7823e8a6" target="_blank" rel="noopener noreferrer">https://medium.com/devgorilla/how-to-log-out-when-using-jwt-a8c7823e8a6<ExternalLinkIcon/></a></li>
<li>CSRF protection with JSON Web JWTs：<a href="https://medium.com/@agungsantoso/csrf-protection-with-json-web-JWTs-83e0f2fcbcc" target="_blank" rel="noopener noreferrer">https://medium.com/@agungsantoso/csrf-protection-with-json-web-JWTs-83e0f2fcbcc<ExternalLinkIcon/></a></li>
<li>Invalidating JSON Web JWTs：<a href="https://stackoverflow.com/questions/21978658/invalidating-json-web-JWTs" target="_blank" rel="noopener noreferrer">https://stackoverflow.com/questions/21978658/invalidating-json-web-JWTs<ExternalLinkIcon/></a></li>
</ul>
</div></template>
