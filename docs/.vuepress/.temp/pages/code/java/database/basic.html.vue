...
<p>数据库系统基本构成如下图所示：</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/e21120184e63406526a4e873cacd23f2.png" alt="数据库系统基本构成" loading="lazy"></p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/5897753dfb301dfa3a814ab06e718a5e.png" alt="关系模型" loading="lazy"></p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/bd1d31be3779342427fc9e462bf7f05c.png" alt="第二范式" loading="lazy"></p>
<p>一些重要的概念：</p>
<ul>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/0fa082bc4d4f919065767476a41b2156.png" alt="阿里巴巴Java开发手册: 禁止存储过程" loading="lazy"></p>
...
<li>delete（删除数据） : <code v-pre>delete from 表名 where 列名=值</code>，删除某一行的数据，如果不加 where 子句和<code v-pre>truncate table 表名</code>作用类似。</li>
</ul>
...
<blockquote>
<p>由于<code v-pre>select</code>不会对表进行破坏，所以有的地方也会把<code v-pre>select</code>单独区分开叫做数据库查询语言DQL（Data Query Language）</p>
</blockquote>
...
<blockquote>
<p><code v-pre>delete</code>命令执行的时候会产生数据库的<code v-pre>binlog</code>日志，而日志记录是需要消耗时间的，但是也有个好处方便数据回滚恢复。</p>
<p><code v-pre>truncate</code>命令执行的时候不会产生数据库日志，因此比<code v-pre>delete</code>要快。除此之外，还会把表的自增值重置和索引恢复到初始大小等。</p>
<p><code v-pre>drop</code>命令会把表占用的空间全部释放掉。</p>
