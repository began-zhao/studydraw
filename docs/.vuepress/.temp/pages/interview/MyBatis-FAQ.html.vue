<template><div><p>MyBatis是一个支持<strong>普通SQL查询</strong>，<strong>存储过程</strong>和<strong>高级映射</strong>的优秀<strong>持久层框架</strong>。MyBatis消除了几乎所有的JDBC代码和参数的手工设置以及对结果集的检索封装。MyBatis可以使用简单的<strong>XML或注解</strong>用于配置和原始映射，将接口和Java的POJO（Plain Old Java Objects，普通的Java对象）映射成数据库中的记录。</p>
<h3 id="jdbc-有几个步骤" tabindex="-1"><a class="header-anchor" href="#jdbc-有几个步骤" aria-hidden="true">#</a> JDBC 有几个步骤？</h3>
<p>JDBC（Java DataBase Connectivity）是 Java 程序与关系型数据库交互的统一 API</p>
<p>JDBC 大致可以分为六个步骤：</p>
<ol>
<li>注册数据库驱动类，指定数据库地址，其中包括 DB 的用户名、密码及其他连接信息；</li>
<li>调用 <code v-pre>DriverManager.getConnection()</code> 方法创建 Connection 连接到数据库；</li>
<li>调用 Connection 的 <code v-pre>createStatement() </code>或 <code v-pre>prepareStatement()</code> 方法，创建 Statement 对象，此时会指定 SQL（或是 SQL 语句模板 + SQL 参数）；</li>
...
<blockquote>
<p>无论是执行查询操作，还是执行其他 DML 操作，1、2、3、4、6 这些步骤都会重复出现。为了简化重复逻辑，提高代码的可维护性，可以将上述重复逻辑封装到一个类似 DBUtils 的工具类中，在使用时只需要调用 DBUtils 工具类中的方法即可。当然，我们也可以使用“反射+配置”的方式，将步骤 5 中关系模型到对象模型的转换进行封装，但是这种封装要做到通用化且兼顾灵活性，就需要一定的编程功底。</p>
...
<p><img src="https://s0.lgstatic.com/i/image/M00/91/5F/CgqCHmAON9CAWnMJAACJd8-3Mcg506.png" alt="" loading="lazy"></p>
...
<p>很多 ORM 框架都支持集成第三方缓存、第三方数据源等常用组件，并对外提供统一的配置接入方式，这样我们只需要使用简单的配置即可完成第三方组件的集成。当我们需要更换某个第三方组件的时候，只需要引入相关依赖并更新配置即可，这就大大提高了开发效率以及整个系统的可维护性。</p>
</blockquote>
<h3 id="什么是-mybatis" tabindex="-1"><a class="header-anchor" href="#什么是-mybatis" aria-hidden="true">#</a> 什么是 Mybatis？</h3>
<p>如果在面试的时候被问到，只要你说出下面三种即可：</p>
<blockquote>
...
<p>MyBatis 封装重复性代码的方式是通过 Mapper 映射配置文件以及相关注解，将 ResultSet 结果映射为 Java 对象，在具体的映射规则中可以嵌套其他映射规则和必要的子查询，这样就可以轻松实现复杂映射的逻辑，当然，也能够实现一对一、一对多、多对多关系映射以及相应的双向关系映射。</p>
<h3 id="什么是-orm" tabindex="-1"><a class="header-anchor" href="#什么是-orm" aria-hidden="true">#</a> 什么是 ORM?</h3>
<p>全称为 Object Relational Mapping。对象-映射-关系型数据库。对象关系映射(简称 ORM，或 O/RM，或 O/R mapping)，用于实现面向对象编程语言里不同类型系统的数据之间的转换。简单的说，ORM 是通过使用描述对象和数据库之间映射的元数据，将程序中的对象与关系数据库相互映射。</p>
<p>ORM 提供了实现持久化层的另一种模式，它采用映射元数据来描述对象关系的映射，使得 ORM 中间件能在任何一个应用的业务逻辑层和数据库层之间充当桥梁。</p>
<h3 id="为什么说mybatis是半自动orm映射工具-它与全自动的区别在哪里" tabindex="-1"><a class="header-anchor" href="#为什么说mybatis是半自动orm映射工具-它与全自动的区别在哪里" aria-hidden="true">#</a> 为什么说Mybatis是半自动ORM映射工具？它与全自动的区别在哪里？</h3>
<p>ORM（Object Relational Mapping），对象关系映射，是一种为了解决关系型数据库数据与简单Java对象（POJO）的映射关系的技术。</p>
...
<h3 id="mybatis优缺点" tabindex="-1"><a class="header-anchor" href="#mybatis优缺点" aria-hidden="true">#</a> Mybatis优缺点</h3>
...
<li>提供映射标签，支持对象与数据库的字段映射；提供对象关系映射标签，支持对象关系组件维护</li>
...
<h3 id="hibernate-和-mybatis-的区别" tabindex="-1"><a class="header-anchor" href="#hibernate-和-mybatis-的区别" aria-hidden="true">#</a> Hibernate 和 MyBatis 的区别</h3>
...
<h3 id="mybatis的工作原理" tabindex="-1"><a class="header-anchor" href="#mybatis的工作原理" aria-hidden="true">#</a> MyBatis的工作原理</h3>
<p><img src="https://tva1.sinaimg.cn/large/e6c9d24ely1h31snxsuxbj20kd0n3abk.jpg" alt="MyBatis框架的执行流程图" loading="lazy"></p>
<ol>
<li>读取 MyBatis 配置文件：mybatis-config.xml 为 MyBatis 的全局配置文件，包含了 MyBatis 行为的设置和属性信息，例如数据库连接信息和映射文件。（我们一般是通过Spring 整合，用 SqlSessionFactoryBean 配置 dataSource 和 mapper 地址等，其实现了InitializingBean 、FactoryBean、ApplicationListener 三个接口 ）</li>
<li>加载映射文件mapper.xml。映射文件即 SQL 映射文件，该文件中配置了操作数据库的 SQL 语句，需要在 MyBatis 配置文件 mybatis-config.xml 中加载。mybatis-config.xml 文件可以加载多个映射文件，每个文件对应数据库中的一张表（MyBatis 会将 Mapper 映射文件中定义的 SQL 语句解析成 SqlSource 对象，其中的动态标签、SQL 语句文本等，会解析成对应类型的 SqlNode 对象）。</li>
...
<h3 id="mybatis的架构设计是怎样的" tabindex="-1"><a class="header-anchor" href="#mybatis的架构设计是怎样的" aria-hidden="true">#</a> MyBatis的架构设计是怎样的</h3>
<p><img src="https://s0.lgstatic.com/i/image2/M01/0B/1B/CgpVE2AT9G2AXu4RAAM4svUMBPc909.png" alt="" loading="lazy"></p>
<p><img src="https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20210809_cd427228-f90f-11eb-8882-00163e068ecd.png" alt="img" loading="lazy"></p>
<p>我们把Mybatis的功能架构分为四层：</p>
<ul>
...
<li>引导层：加载xml配置和Java配置</li>
</ul>
<h3 id="和-的区别" tabindex="-1"><a class="header-anchor" href="#和-的区别" aria-hidden="true">#</a> #{}和${}的区别</h3>
<ul>
<li>#{}是占位符，预编译处理，可以防止SQL注入；${}是拼接符，字符串替换，没有预编译处理，不能防止SQL注入。</li>
<li>Mybatis在处理#{}时，#{}传入参数是以字符串传入，会将SQL中的#{}替换为?号，调用PreparedStatement的set方法来赋值；Mybatis在处理${}时，是原值传入，就是把${}替换成变量的值，相当于JDBC中的Statement编译</li>
<li>#{} 的变量替换是在DBMS 中，变量替换后，#{} 对应的变量自动加上单引号；${} 的变量替换是在 DBMS 外，变量替换后，${} 对应的变量不会加上单引号</li>
</ul>
<h3 id="模糊查询like语句该怎么写" tabindex="-1"><a class="header-anchor" href="#模糊查询like语句该怎么写" aria-hidden="true">#</a> 模糊查询like语句该怎么写</h3>
<ol>
<li>
<p>'%${question}%'  可能引起SQL注入，不推荐</p>
</li>
<li>
<p>&quot;%&quot;#{question}&quot;%&quot;   注意：因为#{...}解析成sql语句时候，会在变量外侧自动加单引号'  '，所以这里 % 需要使用双引号&quot;  &quot;，不能使用单引号 '  '，不然会查不到任何结果。</p>
</li>
<li>
...
　　<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>bind</span> <span class="token attr-name">name</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>pattern<span class="token punctuation">"</span></span> <span class="token attr-name">value</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span><span class="token punctuation">'</span>%<span class="token punctuation">'</span> + username + <span class="token punctuation">'</span>%<span class="token punctuation">'</span><span class="token punctuation">"</span></span> <span class="token punctuation">/></span></span>
　　select id,sex,age,username,password from person where username LIKE #{pattern}
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>select</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="当实体类中的属性名和表中的字段名不一样-怎么办" tabindex="-1"><a class="header-anchor" href="#当实体类中的属性名和表中的字段名不一样-怎么办" aria-hidden="true">#</a> 当实体类中的属性名和表中的字段名不一样 ，怎么办</h3>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>select</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>第2种：通过<code v-pre>&lt;resultMap&gt;</code>
来映射字段名和实体类属性名的一一对应关系。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="什么是mybatis的接口绑定-有哪些实现方式" tabindex="-1"><a class="header-anchor" href="#什么是mybatis的接口绑定-有哪些实现方式" aria-hidden="true">#</a> 什么是MyBatis的接口绑定？有哪些实现方式？</h3>
<p>接口绑定，就是在MyBatis中任意定义接口，然后把接口里面的方法和SQL语句绑定，我们调用接口方法的时候，最终会执行绑定的SQL语句。</p>
<p>接口绑定有两种实现方式，当Sql语句比较简单时候，可以使用注解绑定，当SQL语句比较复杂时候，一般用xml绑定的比较多。</p>
<ul>
<li>通过注解绑定，就是在接口的方法上面加上 @Select、@Update等注解，里面包含Sql语句来实现接口绑定；</li>
<li>通过在xml里面写SQL语句来实现绑定， 在这种情况下，要指定xml映射文件里面的namespace必须为接口的全路径名，同时接口的方法名和SQL语句的id一一对应。</li>
</ul>
<h3 id="使用mybatis的mapper接口调用时有哪些要求" tabindex="-1"><a class="header-anchor" href="#使用mybatis的mapper接口调用时有哪些要求" aria-hidden="true">#</a> 使用MyBatis的mapper接口调用时有哪些要求？</h3>
<ul>
<li>Mapper.xml文件中的namespace即是mapper接口的全限定类名。</li>
<li>Mapper接口方法名和mapper.xml中定义的sql语句id一一对应。</li>
<li>Mapper接口方法的输入参数类型和mapper.xml中定义的每个sql语句的parameterType的类型相同。</li>
<li>Mapper接口方法的输出参数类型和mapper.xml中定义的每个sql语句的resultType的类型相同。</li>
</ul>
<h3 id="mybatis动态sql是做什么的-都有哪些动态sql-能简述一下动态sql的执行原理不" tabindex="-1"><a class="header-anchor" href="#mybatis动态sql是做什么的-都有哪些动态sql-能简述一下动态sql的执行原理不" aria-hidden="true">#</a> MyBatis动态sql是做什么的？都有哪些动态sql？能简述一下动态sql的执行原理不？</h3>
...
<h3 id="mybatis是如何进行分页的-分页插件的原理是什么" tabindex="-1"><a class="header-anchor" href="#mybatis是如何进行分页的-分页插件的原理是什么" aria-hidden="true">#</a> MyBatis是如何进行分页的？分页插件的原理是什么？</h3>
<p>Mybatis使用RowBounds对象进行分页，它是针对ResultSet结果集执行的内存分页，而非物理分页，可以在sql内直接书写带有物理分页的参数来完成物理分页功能，也可以使用分页插件来完成物理分页。</p>
<p>分页插件的基本原理是使用Mybatis提供的插件接口，实现自定义插件，通过jdk动态代理在插件的拦截方法内拦截待执行的sql，然后重写sql，根据dialect方言，添加对应的物理分页语句和参数。</p>
<p>举例：select * from student，拦截sql后重写为：select t.* from (select * from student) t limit 0, 10</p>
<h3 id="mybatis的一级、二级缓存" tabindex="-1"><a class="header-anchor" href="#mybatis的一级、二级缓存" aria-hidden="true">#</a> MyBatis的一级、二级缓存</h3>
<blockquote>
<p>在应用运行过程中，我们有可能在一次数据库会话中，执行多次查询条件完全相同的SQL，MyBatis提供了一级缓存的方案优化这部分场景，如果是相同的SQL语句，会优先命中一级缓存，避免直接对数据库进行查询，提高性能。</p>
<p><img src="https://awps-assets.meituan.net/mit-x/blog-images-bundle-2018a/6e38df6a.jpg" alt="img" loading="lazy"></p>
<p>每个SqlSession中持有了Executor，每个Executor中有一个LocalCache。当用户发起查询时，MyBatis根据当前执行的语句生成<code v-pre>MappedStatement</code>，在Local Cache进行查询，如果缓存命中的话，直接返回结果给用户，如果缓存没有命中的话，查询数据库，结果写入<code v-pre>Local Cache</code>，最后返回结果给用户。具体实现类的类关系图如下图所示。</p>
<p><img src="https://awps-assets.meituan.net/mit-x/blog-images-bundle-2018a/d76ec5fe.jpg" alt="img" loading="lazy"></p>
...
<p><img src="https://tva1.sinaimg.cn/large/e6c9d24ely1h322koh4wxj20qu0dx750.jpg" alt="img" loading="lazy"></p>
<p>当开启缓存后，数据的查询执行的流程就是 二级缓存 -&gt; 一级缓存 -&gt; 数据库</p>
</blockquote>
<ol>
<li>
<p>一级缓存: 基于 PerpetualCache 的 HashMap 本地缓存，其存储作用域为 Session，当 Session flush 或 close 之后，该 Session 中的所有 Cache 就将清空，MyBatis默认打开一级缓存。</p>
</li>
<li>
<p>二级缓存与一级缓存机制相同，默认也是采用 PerpetualCache，HashMap 存储，不同之处在于其存储作用域为 Mapper(Namespace)，并且可自定义存储源，如 Ehcache。默认不打开二级缓存，要开启二级缓存，使用二级缓存属性类需要实现Serializable序列化接口(可用来保存对象的状态)，可在它的映射文件中配置<code v-pre>&lt;cache/&gt;</code>
标签；</p>
</li>
<li>
<p>对于缓存数据更新机制，当某一个作用域(一级缓存 Session/二级缓存Namespaces)进行了C/U/D 操作后，默认该作用域下所有缓存将被清理掉。</p>
</li>
</ol>
<h3 id="通常一个-xml-映射文件-都会写一个-dao-接口与之对应-dao-的工作原理-是否可以重-载" tabindex="-1"><a class="header-anchor" href="#通常一个-xml-映射文件-都会写一个-dao-接口与之对应-dao-的工作原理-是否可以重-载" aria-hidden="true">#</a> 通常一个 Xml 映射文件，都会写一个 Dao 接口与之对应, Dao 的工作原理，是否可以重 载?</h3>
<p>不能重载，因为通过 Dao 寻找 Xml 对应的 sql 的时候全限名+方法名的保存和寻找策略。接口工作原理为 jdk 动态代理原理，运行时会为 dao 生成 proxy，代理对象会拦截接口 方法，去执行对应的 sql 返回数据。</p>
<h3 id="mybatis-中如何执行批处理" tabindex="-1"><a class="header-anchor" href="#mybatis-中如何执行批处理" aria-hidden="true">#</a> Mybatis 中如何执行批处理?</h3>
<p>使用 BatchExecutor 完成批处理。</p>
<h3 id="mybatis-都有哪些-executor-执行器-它们之间的区别是什么" tabindex="-1"><a class="header-anchor" href="#mybatis-都有哪些-executor-执行器-它们之间的区别是什么" aria-hidden="true">#</a> Mybatis 都有哪些 Executor 执行器?它们之间的区别是什么?</h3>
...
<h3 id="resulttype-resultmap-的区别" tabindex="-1"><a class="header-anchor" href="#resulttype-resultmap-的区别" aria-hidden="true">#</a> resultType resultMap 的区别?</h3>
<ul>
<li>
<p>类的名字和数据库相同时，可以直接设置 resultType 参数为 Pojo 类</p>
</li>
<li>
<p>若不同，需要设置 resultMap 将结果名字和 Pojo 名字进行转换</p>
</li>
</ul>
</div></template>
